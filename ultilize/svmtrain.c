...
int read_problem_dense(const mxArray *label_vec, const mxArray *pair_labels, const mxArray * kernel_cache)
{
	int i, j, k, m;
	int elements, max_index, sc, label_vector_row_num;
	double *labels, *kernelcache, *pairlabels;
	...
	if(nrhs > 1 && nrhs < 5)
	{
		...
		if(mxIsSparse(prhs[1]))
		{
			if(param.kernel_type == PRECOMPUTED)
			{
				...
				//prhs[0]: train labels
				//prhs[1]: doublet/triplet element labels
				//prhs[2]: kernel cache
