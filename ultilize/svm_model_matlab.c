...
const char *model_to_matlab_structure(mxArray *plhs[], int num_of_feature, struct svm_model *model)
{
	...
	out_id++;
	// nr_class
	rhs[out_id] = mxCreateDoubleMatrix(1, 1, mxREAL);//1
	ptr = mxGetPr(rhs[out_id]);
	ptr[0] = model->nr_class;
	out_id++;
	// total SV
	rhs[out_id] = mxCreateDoubleMatrix(1, 1, mxREAL);//2
	ptr = mxGetPr(rhs[out_id]);
	ptr[0] = model->l;
	out_id++;
	// rho
	n = model->nr_class*(model->nr_class-1)/2;
	rhs[out_id] = mxCreateDoubleMatrix(n, 1, mxREAL);//3
	ptr = mxGetPr(rhs[out_id]);
	for(i = 0; i < n; i++)
		ptr[i] = model->rho[i];
	out_id++;
	...
	if(model->label)
	{
		rhs[out_id] = mxCreateDoubleMatrix(model->nr_class, 1, mxREAL);//4
		ptr = mxGetPr(rhs[out_id]);
	...
	{
		rhs[out_id] = mxCreateDoubleMatrix(0, 0, mxREAL);//4
	}
	out_id++;
	// probA
/*	if(model->probA != NULL)
	{
		rhs[out_id] = mxCreateDoubleMatrix(n, 1, mxREAL);//5
		ptr = mxGetPr(rhs[out_id]);
		for(i = 0; i < n; i++)
			ptr[i] = model->probA[i];
	}
	...
	{
		mexPrintf("model->probA == NULL\n");
		rhs[out_id] = mxCreateDoubleMatrix(0, 0, mxREAL);//5
	}
	out_id ++;
	mexPrintf("probB\n");
	// probB
	if(model->probB != NULL)
	{
		rhs[out_id] = mxCreateDoubleMatrix(n, 1, mxREAL);//6
		ptr = mxGetPr(rhs[out_id]);
		for(i = 0; i < n; i++)
			ptr[i] = model->probB[i];
	}
	...
	{
		mexPrintf("model->probB == NULL\n");
		rhs[out_id] = mxCreateDoubleMatrix(0, 0, mxREAL);//6
	}
	out_id++;*/
	// nSV
	if(model->nSV)
	{
		rhs[out_id] = mxCreateDoubleMatrix(model->nr_class, 1, mxREAL);//7
		ptr = mxGetPr(rhs[out_id]);
		for(i = 0; i < model->nr_class; i++)
			ptr[i] = model->nSV[i];
	}
	...
	{
		rhs[out_id] = mxCreateDoubleMatrix(0, 0, mxREAL);//7
	}
	out_id++;
	// sv_coef
	rhs[out_id] = mxCreateDoubleMatrix(model->l, model->nr_class-1, mxREAL);//8
	ptr = mxGetPr(rhs[out_id]);
	...
	// SVs
	{
		...
		{
			if (param.kernel_type==MNORM_DOUBLET)
			{
				nonzero_element=2*model->l;
			}else if(param.kernel_type==MNORM_TRIPLET)
			{
				nonzero_element=3*model->l;
			}
			/*nonzero_element = 0;
		...
		//rhs[out_id] = mxCreateSparse(num_of_feature, model->l, nonzero_element, mxREAL);//9
		//return NULL;
		rhs[out_id] = mxCreateDoubleMatrix(num_of_feature, model->l, mxREAL);
		...
		for(i = 0;i < model->l; i++)
		{
			if(model->param.kernel_type == PRECOMPUTED)
			{
				...
				ir_index++;
				//jc[i+1] = jc[i] + 1;
			}else if (model->param.kernel_type == MNORM_DOUBLET){
				int x_index = 0;
			...
			else if (model->param.kernel_type == MNORM_TRIPLET){
				int x_index = 0;
				ptr[ir_index] = model->SV[i][x_index].value;
				ir_index++, x_index++;
				ptr[ir_index] = model->SV[i][x_index].value;
